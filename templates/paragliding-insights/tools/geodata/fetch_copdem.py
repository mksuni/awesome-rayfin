...
from aoi import bbox_wsen, cache_dir, load_aoi, load_world
...
def main() -> None:
    ...
    parser.add_argument(
        "--world",
        help="fetch the union window for a world in config/world/ instead of a single AOI (PLAN §8)",
    )
    ...
    if len(windows) != 1:
        # One 1°×1° tile covers the whole Oberstdorf shell. Mosaicking several is a real case for a
        # larger AOI, but writing it untested and unused would be speculative — so it fails loudly
        # and precisely instead of silently producing a wrong map.
        raise NotImplementedError(
            f"{len(windows)} DEM tiles requested; multi-tile mosaicking is not implemented yet"
        )
